...
function update_progress(status_url, status_div, results) {
  ...
  $.getJSON(status_url, function(data) {
    ...
    if (data['state'] == 'DONE') {
      ...
      d3.json(results, function(error, d) {
        if (error) throw error;

        tabulate('tf1', d, ['rank','factor','summary_p_value','top 100 appearances', 'num samples']);
        // fetch(false);
      ...
      setTimeout(function() {
        update_progress(status_url, status_div, results);
      }, 4500);  // extend to 9000ms for post-process the snakemake results to avoid dataTable issues
    }
  });
}

function start_lisa_task(id, results) {
  ...
  $.ajax({
    ...
    success: function(data, status, request) {
      update_progress(id, div[0], results);
    },
    error: function() {
